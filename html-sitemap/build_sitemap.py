...
def collapse_style(markup: str) -> str:
    """Put the whole stylesheet on one line.
    ...
    discarding everything up to the next ';' or '}' -- which silently wipes
    out most of the stylesheet. With no line breaks there is nothing for it
    to convert, so the CSS survives being pasted into any block.
    """

    def one_line(match: re.Match) -> str:
        css = re.sub(r"/\*.*?\*/", "", match.group(1), flags=re.S)
        css = re.sub(r"\s+", " ", css)
        css = re.sub(r"\s*([{};:,])\s*", r"\1", css)
        return "<style>" + css.strip() + "</style>"

    return re.sub(r"<style>(.*?)</style>", one_line, markup, flags=re.S)
...
TEMPLATE = """<!-- Therapy Near Me - HTML sitemap
     ...
     The CSS below is deliberately on ONE line: WordPress's auto-formatter
     inserts <br /> at every line break, and a <br /> inside a stylesheet
     destroys the rules around it. Keep it on one line when editing.

     Body typography, taken from the site's own text styles - change these
     six values and the whole block follows:
       font-family:"Helvetica",Sans-serif; font-size:16px; font-weight:400;
       font-style:normal; line-height:28px; color:#7A7A7A;
     Headings are deliberately not styled, so they keep the theme's own
     heading font, size and colour. The greys (counts, dates, blurbs,
     borders, toolbar) are mixed from the text colour above, so changing
     `color` re-tints the whole block. Set --tnm-accent for a coloured link
     hover, --tnm-gap for the column gutter. -->
...
def main() -> int:
    ...
    buckets = build_items()
    OUT.write_text(collapse_style(build_html(buckets)), encoding="utf-8")
